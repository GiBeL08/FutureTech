  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white">Управление Подкастами</h2>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#393937] bg-[#141414]">
              ...
              <th className="px-6 py-4 text-left text-white font-semibold">Ведущий</th>
              <th className="px-6 py-4 text-left text-white font-semibold">Эпизодов</th>
              <th className="px-6 py-4 text-left text-white font-semibold">Дата создания</th>
          ...
          <tbody>
            {podcasts.length === 0 ? (
              ...
              podcasts.map((podcast) => (
                <tr key={podcast.id} className="border-b border-[#393937] hover:bg-[#1F1F1F]">
                  <td className="px-6 py-4 text-white">{podcast.title}</td>
                  <td className="px-6 py-4 text-gray-400">{podcast.host || 'N/A'}</td>
                  <td className="px-6 py-4 text-gray-400">{podcast.episodeCount || 0}</td>
                  <td className="px-6 py-4 text-gray-400">
                    {new Date(podcast.createdAt).toLocaleDateString('ru-RU')}
                  </td>
                  <td className="px-6 py-4 flex gap-2">
                    <button className="p-2 hover:bg-[#2A2A2A] rounded-lg text-green-400">
                      <Play size={18} />
                    </button>
